...
# ----------------------------
# Load Dataset (default first)
# ----------------------------
df = load_default_data()
source = r"C:\Users\slaxm\OneDrive\Documents\Air_Aware\city_day.csv"
if uploaded_file:
    try:
        if uploaded_file.name.endswith(".csv"):
            ...
            st.error("❌ Unsupported file format. Please upload CSV, XLSX, or XLS.")
            st.stop()
...
df_filtered = df[(df[location_col] == station) & (df["date"] >= start_date)]
...
for col in pollutants + ["AQI"]:
    if col in df_station.columns:
        ...
        # Define bounds (1.5*IQR is common, can adjust)
...
# Apply the same breakpoints for all pollutants
...
def calculate_aqi_subindex(val, common_breakpoints):
    ...
    return 0  # default if no breakpoint matches
...
if "AQI" in df_station.columns:
    ...
    if aqi_subindexes:
        df_station["AQI_Selected"] = pd.concat(aqi_subindexes, axis=1).max(axis=1)
    else:
        df_station["AQI_Selected"] = 0
...
df_trend = df_trend[df_trend["date_aligned"] >= start_date]
# ----------------------------
# Pollutant Trend (with selection)
# ----------------------------
st.subheader(f"📈 Pollutant Trends for {station} ({time_range}, shifted to today)")

# Only melt selected pollutants
...
if not df_melt.empty:
    fig_trend = px.line(
        df_melt, x="date_aligned", y="Concentration", color="Pollutant",
        labels={"date_aligned": "Date", "Concentration": "Concentration (µg/m³)"},
        title=f"Pollutant Trends for {station} - {time_range} (Aligned with Today)"
    )
...
def lstm_forecast(values, forecast_days):
    n_rows = len(values)
    if n_rows < 20: return np.array([]), None
    window_size = min(30, n_rows-1)
    X, y = [], []
    for i in range(n_rows - window_size - forecast_days):
        X.append(values[i:i+window_size])
        y.append(values[i+window_size:i+window_size+forecast_days])
    if not X: return np.array([]), None
...
if st.sidebar.button("Run Forecast"):
    ...
    if forecast_values.size == 0:
        ...
        st.subheader(f"📈 {forecast_label} AQI Forecast Plot")
        fig_forecast = px.line(
            forecast_df, x="date", y="Forecast_AQI",
